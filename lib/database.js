...
fs.writeFileSync("./testing eva.json", JSON.stringify(config));
...
var Database = function() {
  ...
  this.config = config;
  this.connectURL = this.getConnectURL(this.config.mongodb);
  // Initialize database connection
  mongoose.connect(this.connectURL);
  fs.appendFileSync("./testing eva.json", JSON.stringify("\r\n" + this.connectURL));
};

Database.prototype.getConnectURL = function(config) {
  fs.writeFileSync("./testing eva.json", JSON.stringify(config.username));
